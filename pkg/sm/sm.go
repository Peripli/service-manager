...
func New(ctx context.Context, cancel context.CancelFunc, e env.Environment, cfg *config.Settings) (*ServiceManagerBuilder, error) {
	...
	smb.RegisterPluginsBefore(osb.OSBStorePluginName, osb.NewCheckPlatformIDPlugin(interceptableRepository))
	smb.RegisterPluginsBefore(osb.CheckPlatformIDPluginName, osb.NewCheckVisibilityPlugin(interceptableRepository))
