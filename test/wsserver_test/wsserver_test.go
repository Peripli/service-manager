...
func TestWsConn(t *testing.T) {
	...
	RunSpecs(t, "WS server test suite")
}
...
var _ = Describe("WS", func() {
	...
	JustBeforeEach(func() {
		...
		var err error
		wsconn, resp, err = wsconnect(testContext, "/v1/testws")
	...
	Describe("establish websocket connection", func() {
		...
		Context("when 2 websocket connections are opened", func() {
			...
			JustBeforeEach(func() {
				var err error
				wsconn2, _, err = wsconnect(testContext, "/v1/testws")
				Expect(err).ShouldNot(HaveOccurred())
			})
		...
		// Context("when wsServer is shutdown", func() {
...
func (wsc *wsController) handle(req *web.Request) (*web.Response, error) {
	...
	wsConn, err := wsc.wsServer.Upgrade(rw, req.Request, header, done)
	if err != nil {
		return nil, err
	}
	...
	go handler.Write(wsConn, done)
	go handler.Read(wsConn, done)
...
func (h *wsHandler) Write(c *ws.Conn, done chan<- struct{}) {
	defer func() {
		done <- struct{}{}
	}()
...
func (h *wsHandler) Read(c *ws.Conn, done chan<- struct{}) {
	defer func() {
		done <- struct{}{}
		c.Close()
	}()
