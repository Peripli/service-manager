	...
	AdditionalTests: func(ctx *common.TestContext) {
		Context("non-generic tests", func() {
			...
			BeforeEach(func() {
				...
				existingPlanIDs = common.List(ctx.SMWithOAuth, web.ServicePlansURL).Path("$[*].id").Array().Raw()
			...
			Describe("POST", func() {
				...
				Context("with missing platform id field", func() {
					It("returns 201 if no visibilities for the plan exist", func() {
						common.List(ctx.SMWithOAuth, web.VisibilitiesURL).Path("$[*].id").Array().NotContains(existingPlanIDs[1])
					...
					It("returns 400 if visibilities for the plan exist", func() {
						...
						common.List(ctx.SMWithOAuth, web.VisibilitiesURL).Path("$[*].service_plan_id").Array().Contains(existingPlanIDs[0])
...
func blueprint(setNullFieldsValues bool) func(ctx *common.TestContext, auth *httpexpect.Expect) common.Object {
	return func(ctx *common.TestContext, auth *httpexpect.Expect) common.Object {
		...
		servicePlanID := common.ListWithQuery(auth, web.ServicePlansURL, "fieldQuery="+fmt.Sprintf("service_offering_id = %s", so.Object().Value("id").String().Raw())).First().Object().Value("id").String().Raw()
