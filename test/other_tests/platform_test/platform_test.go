...
var _ = test.DescribeTestsFor(test.TestCase{
	...
	AdditionalTests: func(ctx *common2.TestContext, t *test.TestCase) {
		Context("non-generic tests", func() {
			BeforeEach(func() {
				common2.RemoveAllPlatforms(ctx.SMRepository)
			})

			Describe("POST", func() {
				Context("With 2 platforms", func() {
					BeforeEach(func() {
						platformJSON := common2.GenerateRandomPlatform()
						platformJSON["name"] = "k"
						common2.RegisterPlatformInSM(platformJSON, ctx.SMWithOAuth, nil)

						platformJSON2 := common2.GenerateRandomPlatform()
						platformJSON2["name"] = "a"
						common2.RegisterPlatformInSM(platformJSON2, ctx.SMWithOAuth, nil)
					})
				...
				Context("With missing mandatory fields", func() {
					It("returns 400", func() {
						newplatform := func() common2.Object {
							return common2.MakePlatform("platform1", "cf-10", "cf", "descr")
						}
						ctx.SMWithOAuth.POST(web.PlatformsURL).
				...
				Context("With conflicting fields", func() {
					It("returns 409", func() {
						platform := common2.MakePlatform("platform1", "cf-10", "cf", "descr")
						ctx.SMWithOAuth.POST(web.PlatformsURL).
							WithJSON(platform).
							Expect().Status(http.StatusCreated)
						ctx.SMWithOAuth.POST(web.PlatformsURL).
				...
				Context("With optional fields skipped", func() {
					It("succeeds", func() {
						...
						common2.MapContains(reply.Raw(), platform)
					})
				})

				Context("With invalid id", func() {
					It("fails", func() {
						platform := common2.MakePlatform("platform/1", "cf-10", "cf", "descr")
				...
				Context("Without id", func() {
					It("returns the new platform with generated id and credentials", func() {
						platform := common2.MakePlatform("", "cf-10", "cf", "descr")
						delete(platform, "id")
			...
			Describe("PATCH", func() {
				var platform common2.Object
				const id = "p1"

				BeforeEach(func() {
					By("Create new platform")

					platform = common2.MakePlatform(id, "cf-10", "cf", "descr")
					ctx.SMWithOAuth.POST(web.PlatformsURL).
						WithJSON(platform).
						Expect().Status(http.StatusCreated)
				})
				...
				Context("With all properties updated", func() {
					It("returns 200", func() {
						...
						updatedPlatform := common2.MakePlatform("", "cf-11", "cff", "descr2")
						...
						common2.MapContains(reply.Raw(), updatedPlatform)
				...
				Context("With created_at in body", func() {
					It("should not update created_at", func() {
						...
						updatedPlatform := common2.Object{
				...
				Context("With provided id", func() {
					It("should not update platform id", func() {
						ctx.SMWithOAuth.PATCH(web.PlatformsURL + "/" + id).
							WithJSON(common2.Object{"id": "123"}).
							Expect().
							Status(http.StatusOK).JSON().Object().
							NotContainsKey("credentials")
				...
				Context("On missing platform", func() {
					It("returns 404", func() {
						ctx.SMWithOAuth.PATCH(web.PlatformsURL + "/123").
							WithJSON(common2.Object{"name": "123"}).
							Expect().
							Status(http.StatusNotFound)
					})
				})
				...
				Context("With conflicting fields", func() {
					It("should return 409", func() {
						platform2 := common2.MakePlatform("p2", "cf-12", "cf2", "descr2")
						ctx.SMWithOAuth.POST(web.PlatformsURL).
							WithJSON(platform2).
							Expect().Status(http.StatusCreated)
			...
			Describe("DELETE", func() {
				...
				BeforeEach(func() {
					platform = common2.MakePlatform(platformID, "cf-10", "cf", "descr")
					ctx.SMWithOAuth.POST(web.PlatformsURL).
						WithJSON(platform).
						Expect().Status(http.StatusCreated)

					common2.CreateInstanceInPlatform(ctx, platformID)
				})
...
func blueprint(setNullFieldsValues bool) func(ctx *common2.TestContext, auth *common2.SMExpect, async bool) common2.Object {
	return func(_ *common2.TestContext, auth *common2.SMExpect, _ bool) common2.Object {
		randomPlatform := common2.GenerateRandomPlatform()
