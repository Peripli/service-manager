...
var _ = Describe("Service Manager Plugins", func() {
	...
	Describe("Partial plugin", func() {
		BeforeEach(func() {
			...
			planID = UUID.String()
			plan1 := common2.GenerateTestPlanWithID(planID)
			UUID, err = uuid.NewV4()
			Expect(err).ToNot(HaveOccurred())
			serviceID = UUID.String()
			service1 := common2.GenerateTestServiceWithPlansWithID(serviceID, plan1)
			...
			common2.CreateVisibilitiesForAllBrokerPlans(ctx.SMWithOAuth, brokerID)
