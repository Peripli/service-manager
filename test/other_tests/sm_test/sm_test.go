...
var _ = Describe("SM", func() {
	var (
		...
		oauthServer *common2.OAuthServer
		fakeEnv     *envfakes.FakeEnvironment
	)

	BeforeEach(func() {
		ctx, cancel = context.WithCancel(context.Background())
		oauthServer = common2.NewOAuthServer()
		fakeEnv = &envfakes.FakeEnvironment{}
	})
	...
	Describe("New", func() {
		Context("when validating config fails", func() {
			It("should return error", func() {
				env, err := env.Default(context.TODO(), config.AddPFlags, common2.SetTestFileLocation)
		...
		Context("Service Manager platform", func() {
			...
			BeforeEach(func() {
				ctx = common2.NewTestContextBuilder().Build()
			...
			auths := []func() *common2.SMExpect{
				func() *common2.SMExpect {
					return ctx.SM
				},
				func() *common2.SMExpect {
			...
			for _, auth := range auths {
				...
				It("disallows patching the service manager platform", func() {
					auth().PATCH(web.PlatformsURL + "/" + smPlatformID).WithJSON(common2.Object{}).
						Expect().Status(http.StatusNotFound)
				})
			}
		})
...
func verifyServiceManagerStartsSuccessFully(serviceManagerServer *httptest.Server) *common2.SMExpect {
