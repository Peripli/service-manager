...
var _ = Describe("Interceptors", func() {
	...
	BeforeSuite(func() {
		...
		contextBuilder := common2.NewTestContextBuilderWithSecurity()
	...
	Describe("Calling other intereceptors", func() {
		Context("when interceptor fails", func() {
			It("should cancel the transaction", func() {
				...
				platform2 := common2.GenerateRandomPlatform()
	...
	Describe("Positioning", func() {
		...
		Context("Broker", func() {
			It("Should call interceptors in right order", func() {
				brokerID := ctx.RegisterBrokerWithCatalog(common2.NewRandomSBCatalog()).Broker.ID // Post /v1/service_brokers
				checkCreateStack(types.ServiceBrokerType)

				ctx.SMWithOAuth.PATCH(web.ServiceBrokersURL + "/" + brokerID).WithJSON(common2.Object{}).Expect().Status(http.StatusOK)
		...
		Context("Platform", func() {
			It("Should call interceptors in right order", func() {
				...
				ctx.SMWithOAuth.PATCH(web.PlatformsURL + "/" + platform.ID).WithJSON(common2.Object{}).Expect().Status(http.StatusOK)
				checkUpdateStack(types.PlatformType)

				ctx.SMWithOAuth.DELETE(web.PlatformsURL + "/" + platform.ID).WithJSON(common2.Object{}).Expect().Status(http.StatusOK)
		...
		Context("Visibilities", func() {
			It("Should call interceptors in right order", func() {
				...
				ctx.SMWithOAuth.PATCH(web.VisibilitiesURL + "/" + visibilityID).WithJSON(common2.Object{}).Expect().Status(http.StatusOK)
		...
		for _, e := range entries {
			Context(e.name, func() {
				...
				It("Update", func() {
					...
					entityID := e.createEntryFunc()
					ctx.SMWithOAuth.PATCH(e.url + "/" + entityID).WithJSON(common2.Object{}).
						Expect().JSON().Object().
						Value("labels").Object().
						Value(newLabelKey).Array().Equal(newLabelValue)
				})
