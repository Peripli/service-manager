...
var _ = Describe("Service Manager Public Plans Interceptor", func() {
	...
	var existingBrokerServer *common2.BrokerServer
	...
	BeforeEach(func() {
		...
		c := common2.NewEmptySBCatalog()

		oldPublicPlan := common2.GenerateFreeTestPlan()
		oldPaidPlan = common2.GeneratePaidTestPlan()
		newPublicPlan = common2.GenerateFreeTestPlan()
		newPaidPlan = common2.GeneratePaidTestPlan()
		oldService := common2.GenerateTestServiceWithPlans(oldPublicPlan, oldPaidPlan)
		...
		existingBrokerServer.Catalog = common2.SBCatalog(testCatalog)
	...
	Context("when the catalog is empty", func() {
		...
		BeforeEach(func() {
			id, _, _ = ctx.RegisterBrokerWithCatalog(common2.NewEmptySBCatalog()).GetBrokerAsParams()
	...
	Context("when a new public plan is added", func() {
		BeforeEach(func() {
			s, err := sjson.Set(testCatalog, "services.0.plans.-1", common2.JSONToMap(newPublicPlan))
			Expect(err).ShouldNot(HaveOccurred())
			existingBrokerServer.Catalog = common2.SBCatalog(s)
		})
	...
	Context("when a new paid plan is added", func() {
		BeforeEach(func() {
			s, err := sjson.Set(testCatalog, "services.0.plans.-1", common2.JSONToMap(newPaidPlan))
			Expect(err).ShouldNot(HaveOccurred())
			existingBrokerServer.Catalog = common2.SBCatalog(s)
		})
	...
	Context("when an existing public plan is made paid", func() {
		BeforeEach(func() {
			...
			existingBrokerServer.Catalog = common2.SBCatalog(catalog)
		})
	...
	Context("when a plan has specified supported platform names", func() {
		...
		Context("when a broker has a plan supporting platform names and another supporting platform types", func() {
			...
			JustBeforeEach(func() {
				...
				newPlan := common2.GenerateFreeTestPlan()
				...
				var catalog string
				catalog, err = sjson.Set(string(existingBrokerServer.Catalog), "services.0.plans.-1", common2.JSONToMap(additionalPublicPlan))
				Expect(err).ShouldNot(HaveOccurred())
	...
	Context("when a plan has specified excluded platform names", func() {
		...
		Context("when a broker has a plan excluding a platforms and another supporting all platforms", func() {
			...
			JustBeforeEach(func() {
				...
				catalog, err := sjson.Set(string(existingBrokerServer.Catalog), "services.0.plans.-1", common2.JSONToMap(newPlan))
				Expect(err).ShouldNot(HaveOccurred())
