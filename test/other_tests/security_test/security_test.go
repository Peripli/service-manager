...
var _ = Describe("Service Manager Security Tests", func() {
	...
	BeforeEach(func() {
		contextBuilder = common2.NewTestContextBuilder().WithBasicAuthPlatformName("security-tests-platform")
	})
...
func attachBearer(contextBuilder *common2.TestContextBuilder, paths []string, methods []string, required bool) {
	contextBuilder.WithSMExtensions(func(_ context.Context, smb *sm.ServiceManagerBuilder, e env.Environment) error {
...
func attachRequiredBearer(contextBuilder *common2.TestContextBuilder, paths []string, methods []string) {
	attachBearer(contextBuilder, paths, methods, true)
}

func attachOptionalBearer(contextBuilder *common2.TestContextBuilder, paths []string, methods []string) {
	attachBearer(contextBuilder, paths, methods, false)
}

func attachRequiredBasic(contextBuilder *common2.TestContextBuilder, paths []string, methods []string, authenticatorFunc authenticators.BasicAuthenticatorFunc) {
	attachBasic(contextBuilder, paths, methods, authenticatorFunc, true)
}

func attachOptionalBasic(contextBuilder *common2.TestContextBuilder, paths []string, methods []string, authenticatorFunc authenticators.BasicAuthenticatorFunc) {
	attachBasic(contextBuilder, paths, methods, authenticatorFunc, false)
}

func attachBasic(contextBuilder *common2.TestContextBuilder, paths []string, methods []string, authenticatorFunc authenticators.BasicAuthenticatorFunc, required bool) {
	contextBuilder.WithSMExtensions(func(_ context.Context, smb *sm.ServiceManagerBuilder, e env.Environment) error {
		secState := smb.Security().Path(paths...).Method(methods...).
