...
var (
	ctx                 *common2.TestContext
	SMWithBasicPlatform *common2.SMExpect

	brokerServerWithEmptyCatalog *common2.BrokerServer
	emptyCatalogBrokerID         string
	smUrlToEmptyCatalogBroker    string

	brokerServerWithSimpleCatalog    *common2.BrokerServer
	simpleBrokerCatalogID            string
	smUrlToSimpleBrokerCatalogBroker string

	stoppedBrokerServer  *common2.BrokerServer
	stoppedBrokerID      string
	smUrlToStoppedBroker string

	brokerServer *common2.BrokerServer
	brokerID     string
	brokerName   string
	smBrokerURL  string
...
var _ = BeforeSuite(func() {
	ctx = common2.NewTestContextBuilderWithSecurity().WithEnvPreExtensions(func(set *pflag.FlagSet) {
	...
	SMWithBasicPlatform = &common2.SMExpect{Expect: ctx.SMWithBasic.Expect}

	brokerPlatformCredentialsIDMap = make(map[string]brokerPlatformCredentials)

	butils := ctx.RegisterBrokerWithCatalog(common2.NewEmptySBCatalog())
	...
	common2.CreateVisibilitiesForAllBrokerPlans(ctx.SMWithOAuth, simpleBrokerCatalogID)
	...
	plan0 := common2.GenerateTestPlanWithID(plan0CatalogID)
	service0 := common2.GenerateTestServiceWithPlansWithID(service0CatalogID, plan0)
	...
	common2.CreateVisibilitiesForAllBrokerPlans(ctx.SMWithOAuth, stoppedBrokerID)
	...
	plan1 := common2.GenerateTestPlanWithID(plan1CatalogID)
	plan2 := common2.GenerateTestPlanWithID(plan2CatalogID)
	plan3 := common2.GenerateTestPlanWithID(plan3CatalogID)

	service1 := common2.GenerateTestServiceWithPlansWithID(service1CatalogID, plan1, plan2, plan3)
	catalog = common2.NewEmptySBCatalog()
	catalog.AddService(service1)

	var brokerObject common2.Object
	...
	for _, p := range plans {
		common2.RegisterVisibilityForPlanAndPlatform(ctx.SMWithOAuth, p.Object().Value("id").String().Raw(), ctx.TestPlatform.ID)
	}
	smBrokerURL = ctx.Servers[common2.SMServer].URL() + "/v1/osb/" + brokerID
...
var _ = JustAfterEach(func() {
	common2.RemoveAllOperations(ctx.SMRepository)
	common2.RemoveAllBindings(ctx)
	common2.RemoveAllInstances(ctx)
	common2.RemoveAllOperations(ctx.SMRepository)
})
...
func queryParameterVerificationHandler(key, value string) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		...
		common2.SetResponse(writer, status, common2.Object{})
	}
}
...
func delayingHandler(done chan<- interface{}) func(rw http.ResponseWriter, req *http.Request) {
	return func(rw http.ResponseWriter, req *http.Request) {
		...
		<-timeoutContext.Done()
		common2.SetResponse(rw, http.StatusTeapot, common2.Object{})
		close(done)
	}
}
...
func provisionRequestBodyMapWith(key, value string, idsToRemove ...string) func() map[string]interface{} {
	return func() map[string]interface{} {
		...
		return common2.JSONToMap(provisionRequestBody)
	}
}
