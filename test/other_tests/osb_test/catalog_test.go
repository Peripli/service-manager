...
var _ = Describe("Catalog", func() {
	...
	Describe("filtering catalog for k8s platform based on visibilities", func() {
		...
		assertBrokerPlansVisibleForPlatform := func(brokerID string, agent *common2.SMExpect, plans ...interface{}) {
		...
		BeforeEach(func() {
			...
			plan1 = common2.GenerateTestPlan()
			plan2 = common2.GenerateTestPlan()
			plan3 = common2.GenerateTestPlan()
			service1 := common2.GenerateTestServiceWithPlans(plan1, plan2)
			catalog.AddService(service1)
			service2 := common2.GenerateTestServiceWithPlans(plan3)
		...
		Context("for platform with visibilities for 2 plans from 2 services", func() {
			It("should return 2 plans", func() {
				...
				ctx.SMWithOAuth.POST(web.VisibilitiesURL).WithJSON(common2.Object{
	...
	Describe("Prefixed broker path", func() {
		Context("when call to working broker", func() {
			...
			var (
				server           common2.FakeServer
				oldSMWithBasic   *common2.SMExpect
				osbURL           string
				prefixedBrokerID string
			)

			assertCredentialsNotChanged := func(oldSMExpect, newSMExpect *common2.SMExpect) {
			...
			BeforeEach(func() {
				...
				prefixedBrokerID = common2.RegisterBrokerInSM(brokerJSON, ctx.SMWithOAuth, map[string]string{})["id"].(string)
				ctx.Servers[common2.BrokerServerPrefix+prefixedBrokerID] = server
				...
				oldSMWithBasic = &common2.SMExpect{Expect: ctx.SMWithBasic.Expect}
			...
			Context("when broker platform credentials change out of notification processing context when already exist", func() {
				It("should return 409 for non-kubernetes platforms", func() {
					...
					k8sPlatformClient := &common2.SMExpect{Expect: ctx.SM.Builder(func(req *httpexpect.Request) {
						username, password := k8sPlatform.Credentials.Basic.Username, k8sPlatform.Credentials.Basic.Password
						req.WithBasicAuth(username, password).WithClient(ctx.HttpClient)
					})}

					username, password := test.RegisterBrokerPlatformCredentials(k8sPlatformClient, prefixedBrokerID)
					k8sOSBClient := &common2.SMExpect{Expect: ctx.SM.Builder(func(req *httpexpect.Request) {
						req.WithBasicAuth(username, password).WithClient(ctx.HttpClient)
					})}
		...
		When("registering broker which is not actually an OSB complient broker application", func() {
			...
			It("should not disclose internal information if response code is bad request", func() {
				...
				ctx.SMWithOAuth.POST(web.ServiceBrokersURL).WithJSON(common2.Object{
					"broker_url": brokerURL,
					"name":       "not_broker",
					"credentials": common2.Object{
						"basic": common2.Object{
...
func (pbh *prefixedBrokerHandler) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	if strings.HasPrefix(req.URL.Path, pbh.brokerPathPrefix) {
		...
		common2.SetResponse(w, http.StatusOK, servicesMap)
	} else {
		common2.SetResponse(w, http.StatusNotFound, common2.Object{})
	}
}
