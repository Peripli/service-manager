...
var _ = Describe("Post Binding Adapt Credentials", func() {
	Context("when call to working service broker", func() {
		It("should succeed", func() {
			...
			ctx.SMWithBasic.POST(smBrokerURL+"/v2/service_instances/iid/service_bindings/bid/adapt_credentials").WithHeader(brokerAPIVersionHeaderKey, brokerAPIVersionHeaderValue).WithJSON(&common2.Object{}).
	...
	Context("when call to missing service broker", func() {
		It("should fail", func() {
			assertMissingBrokerError(
				ctx.SMWithBasic.POST("http://localhost:3456/v1/osb/123"+"/v2/service_instances/iid/service_bindings/bid/adapt_credentials").WithHeader(brokerAPIVersionHeaderKey, brokerAPIVersionHeaderValue).WithJSON(&common2.Object{}).Expect())
	...
	Context("when call to stopped service broker", func() {
		It("should fail", func() {
			assertUnresponsiveBrokerError(
				ctx.SMWithBasic.POST(smUrlToStoppedBroker+"/v2/service_instances/iid/service_bindings/bid/adapt_credentials").WithHeader(brokerAPIVersionHeaderKey, brokerAPIVersionHeaderValue).WithJSON(&common2.Object{}).Expect())
		})
	})
