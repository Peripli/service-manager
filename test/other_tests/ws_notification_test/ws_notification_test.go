...
var _ = Describe("WS", func() {
	...
	wsconnectWithPlatform := func(queryParams map[string]string) (*types.Platform, *websocket.Conn, *http.Response, error) {
		platform := common2.RegisterPlatformInSM(common2.GenerateRandomPlatform(), ctx.SMWithOAuth, map[string]string{})
	...
	BeforeEach(func() {
		...
		ctx = common2.NewTestContextBuilderWithSecurity().
	...
	Context("platform health", func() {
		...
		BeforeEach(func() {
			pongCh = make(chan struct{})
			newPlatform = common2.RegisterPlatformInSM(common2.GenerateRandomPlatform(), ctx.SMWithOAuth, map[string]string{})
...
func createNotification(repository storage.Repository, platformID string) *types.Notification {
	notification := common2.GenerateRandomNotification()
