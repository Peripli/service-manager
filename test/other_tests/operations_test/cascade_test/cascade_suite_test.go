...
var _ = BeforeSuite(func() {
	...
	ctx = common.NewTestContextBuilderWithSecurity().WithEnvPostExtensions(postHook).
...
func createOSBInstance(ctx *common.TestContext, sm *common.SMExpect, brokerID string, instanceID string, osbContext map[string]interface{}) {
...
func createSMAAPInstance(ctx *common.TestContext, sm *common.SMExpect, context map[string]interface{}) string {
	smBrokerURL := ctx.Servers[common.SMServer].URL()
...
func createSMAAPBinding(ctx *common.TestContext, sm *common.SMExpect, context map[string]interface{}) {
	smBrokerURL := ctx.Servers[common.SMServer].URL()
...
func createOSBBinding(ctx *common.TestContext, sm *common.SMExpect, brokerID string, instanceID string, bindingID string, osbContext map[string]interface{}) {
	smBrokerURL := ctx.Servers[common.SMServer].URL() + "/v1/osb/" + brokerID
...
func registerSubaccountScopedBroker(ctx *common.TestContext, serviceNameID string, planID string) (string, *common.BrokerServer) {
	...
	common.CreateVisibilitiesForAllBrokerPlans(ctx.SMWithOAuth, id)
	return id, brokerServer
}

func registerInstanceLastOPHandlers(brokerServer *common.BrokerServer, status int, state types.OperationState) {
	brokerServer.ServiceInstanceLastOpHandlerFunc(http.MethodDelete+"1", func(req *http.Request) (int, map[string]interface{}) {
		if status == http.StatusOK {
			return status, common.Object{"state": state}
		} else {
			return status, common.Object{}
		}
	})
}

func registerBindingLastOPHandlers(brokerServer *common.BrokerServer, status int, state types.OperationState) {
...
func registerSubaccountScopedPlatform(ctx *common.TestContext, name string) string {
	platform := common.MakePlatform(name, name, "cf", "descr")
