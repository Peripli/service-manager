...
var _ = Describe("Catalog CatalogFetcher", func() {
	...
	testBrokerTls := types.ServiceBroker{
		Base: types.Base{
			ID:     id,
			Labels: map[string][]string{},
			Ready:  true,
	...
	newFetcher := func(t testCase) func(ctx context.Context, broker *types.ServiceBroker) ([]byte, error) {
		return osb.CatalogFetcher(common.DoHTTPWithClient(t.reaction, t.expectations), version, util.TransportWithTlsProvider(&http.Transport{}))
	}
