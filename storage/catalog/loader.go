...
func Load(ctx context.Context, brokerID string, repository storage.Repository) (*types.ServiceOfferings, error) {
	serviceOfferings, err := repository.List(ctx, types.ServiceOfferingType, nil, query.ByField(query.EqualsOperator, "broker_id", brokerID))
	if err != nil {
		return nil, err
	}
	...
	if len(serviceOfferingIDs) > 0 {
		servicePlans, err := repository.List(ctx, types.ServicePlanType, nil, query.ByField(query.InOperator, "service_offering_id", serviceOfferingIDs...))
		if err != nil {
			return nil, err
		}
