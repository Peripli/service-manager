...
var _ = Describe("Secured Storage", func() {
	...
	BeforeEach(func() {
		...
		mock.ExpectQuery(`SELECT version, dirty FROM "schema_migrations" LIMIT 1`).WillReturnRows(sqlmock.NewRows([]string{"version", "dirty"}).FromCSVString("20200114111100,false"))
