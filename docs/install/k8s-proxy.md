...
```console
helm install charts/service-broker-proxy --service-broker-proxy --namespace service-broker-proxy --set config.sm.url=<SM_URL> --set sm.user=<SM_USER> --set sm.password=<SM_PASSWORD>
```
